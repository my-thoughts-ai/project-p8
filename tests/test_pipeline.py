...
def test_danger_mode_does_not_repeat_itself(tmp_path, monkeypatch):
    ...
    # This test exercises de-duplication, not threshold calibration. The captured
    # fixture is a calm real forecast that is green under the production thresholds,
    # so force a red here with a deliberately sensitive precip cut-off.
    cfg["thresholds"]["precipitation_mm_per_h"] = {"amber": 0.01, "red": 0.1}
    monkeypatch.setattr("src.main.load_config", lambda path=None: cfg)
...
def test_configured_sms_expiry_is_the_agreed_date(cfg):
    assert cfg["channels"]["sms"]["active_until"] == "2026-11-20"
