    ...
    for hours in cfg["digest"]["table_hours"]:
        ...
        for _, row in slice_.iterrows():
            rows.append({
                ...
                "storm": row.get("storm", "No storm"),
                "storm_on": (row.get("storm") == "Storm"),
            })
        first = slice_.iloc[0]
        blocks.append({
...
def headline(frame: pd.DataFrame, windows: List[Dict[str, Any]], overall: str) -> str:
    ...
    # Describe the next 24 h, not the 7-day worst, so the label matches the numbers.
...
def render_email_text(context: Dict[str, Any]) -> str:
    ...
    for block in context["blocks"]:
        ...
        lines.append("  {:<10} {:>9} {:>11} {:>6} {:>11} {:>6}  {:<6}  {}".format(
            "ALTITUDE", "TEMP", "WIND", "DIR", "CHILL", "RH", "STATUS", "STORM"))
...
def render_telegram(context: Dict[str, Any]) -> str:
    ...
    for block in context["blocks"][:4]:
        ...
        lines.append("<pre>" + "\n".join(
            "{:<7} {:>7} {:>9} {:>4} {:<6} {}".format(
                str(r["altitude"]) + "m", r["temp"], r["wind"], r["direction"],
                r["status"], "STORM" if r["storm_on"] else "-")
