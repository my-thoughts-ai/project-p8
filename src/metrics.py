...
STORM, NO_STORM = "Storm", "No storm"


def storm_status(row: pd.Series, storm_cfg: Dict[str, Any]) -> str:
    """Per-band storm flag: 'Storm' if any storm criterion trips, else 'No storm'.

    Storm is mostly a column-wide phenomenon (weather code, CAPE, pressure fall,
    heavy precip/snow) but wind is genuinely per-altitude, so the summit can read
    Storm on wind while base camp does not. All thresholds live in config.storm.
    """
    if not storm_cfg:
        return NO_STORM
    ...
    if code is not None and pd.notna(code):
        if int(code) in set(storm_cfg.get("weather_codes", [])):
            return STORM

    def over(field: str, key: str) -> bool:
        value = row.get(field)
        limit = storm_cfg.get(key)
        return (value is not None and pd.notna(value) and limit is not None
                and float(value) >= float(limit))

    if over("cape", "cape_j_per_kg"):
        return STORM
    if over("precipitation", "precipitation_mm_per_h"):
        return STORM
    if over("snowfall", "snowfall_mm_per_h"):
        return STORM
    if over("wind_speed_kmh", "wind_kmh"):
        return STORM

    drop = row.get("mslp_change_6h")
    limit = storm_cfg.get("mslp_drop_hpa_per_6h")
    if drop is not None and pd.notna(drop) and limit is not None and -float(drop) >= float(limit):
        return STORM
...
def evaluate(frame: pd.DataFrame, cfg: Dict[str, Any]) -> pd.DataFrame:
    ...
    def row_status(row: pd.Series) -> pd.Series:
        ...
        return pd.Series({
            ...
            "storm": storm_status(row, storm_cfg),
        })
